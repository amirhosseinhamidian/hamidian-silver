...
export async function verifyOtp(request: Request): Promise<Response> {
  ...
  if (!currentUserResult.response.ok) {
    return responseFromApi(currentUserResult.response, currentUserResult.error);
  }
  ...
  if (!hasAdministrativeAccess(currentUserResult.data)) {
    return Response.json({ message: 'Administrative access required.' }, { status: 403 });
  }
...
export async function getCurrentUser(): Promise<Response> {
  ...
  if (!hasAdministrativeAccess(data)) {
    await clearSessionCookie();
    return Response.json({ message: 'Administrative access required.' }, { status: 403 });
  }
