...
export async function forwardRefundMutation(request: Request, apiPath: string): Promise<Response> {
  ...
  try {
    ...
    return Response.json({ message: 'Refund service is unavailable.' }, { status: 502 });
  }
}
