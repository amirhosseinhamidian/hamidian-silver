...
export async function forwardReturnMutation(request: Request, apiPath: string): Promise<Response> {
  ...
  try {
    ...
    return Response.json({ message: 'Return service is unavailable.' }, { status: 502 });
  }
}
