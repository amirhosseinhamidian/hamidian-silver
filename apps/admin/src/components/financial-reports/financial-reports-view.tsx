...
function CashflowPanel({ data }: Readonly<{ data: AdminFinancialReportsData }>) {
  ...
  if (!cashflow) {
    return (
      <Card title="جریان نقدی" description="ورودی مشتری و خروجی‌های قطعی دوره">
        <ResourceAlert resource={data.cashflow}>
          داده جریان نقدی از سرویس مالی دریافت نشد.
        </ResourceAlert>
      </Card>
    );
  }
  const rows = [
    {
      label: 'دریافت از مشتری',
      value: cashflow.customerCashInToman,
      color: 'var(--admin-color-success)',
    },
    {
      label: 'بازپرداخت مشتری',
      value: cashflow.customerRefundCashOutToman,
      color: 'var(--admin-color-danger)',
    },
    {
      label: 'پرداخت نقدی تأمین‌کننده',
      value: cashflow.supplierCashOutToman,
      color: 'var(--admin-color-warning)',
    },
    {
      label: 'جریان نقد عملیاتی خالص',
      value: cashflow.netOperatingCashflowToman,
  ...
  return (
    <Card title="جریان نقدی" description="ورودی مشتری و خروجی‌های قطعی دوره">
      <div role="img" aria-label="نمودار جریان نقدی دوره" className="space-y-4">
        {rows.map((row) => (
          <FlowBar key={row.label} {...row} max={max} />
        ))}
      </div>
      <dl className="mt-5 grid grid-cols-2 gap-2 border-t border-[var(--admin-color-border)] pt-4 text-xs">
        <SmallStat
          label="سفارش پرداخت‌شده"
          value={formatAdminInteger(cashflow.customerCashInOrderCount)}
        />
        <SmallStat
          label="تعداد بازپرداخت"
          value={formatAdminInteger(cashflow.customerRefundCount)}
        />
        <SmallStat
          label="دوره تسویه پرداخت‌شده"
          value={formatAdminInteger(cashflow.supplierSettlementCount)}
        />
        <SmallStat
          label="پرداخت مستقیم بدهی"
          value={formatAdminInteger(cashflow.supplierDirectPaymentCount)}
        />
      </dl>
    </Card>
  );
  ...
  if (!contribution) {
    return (
      <Card title="حاشیه مشارکت" description="سود پس از هزینه‌های خدمات و بازپرداخت">
        <ResourceAlert resource={data.contribution}>
          داده سود و هزینه از سرویس مالی دریافت نشد.
        </ResourceAlert>
      </Card>
    );
  }
  ...
  return (
    <Card title="حاشیه مشارکت" description="سود پس از هزینه‌های خدمات و بازپرداخت">
      <dl className="space-y-3 text-sm">
        <ReportLine
          label="سود ناخالص پیش از خدمات"
          value={contribution.grossProfitBeforeServiceCostsToman}
        />
        ...
        <ReportLine
          label="مشارکت خالص پس از بازپرداخت"
          value={contribution.contributionAfterRefundsToman}
          strong
        />
      </dl>
      <div className="mt-5 border-t border-[var(--admin-color-border)] pt-4">
        <DonutChart
          ...
          segments={[
            {
              label: 'کارمزد درگاه',
              value: Math.max(0, contribution.paymentGatewayFeeToman),
              color: '#2563eb',
            },
            {
              label: 'ارسال',
              value: Math.max(0, contribution.shippingProviderCostToman),
              color: '#d97706',
            },
            {
              label: 'آبکاری',
              value: Math.max(0, contribution.platingServiceCostToman),
              color: '#7c3aed',
            },
            {
              label: 'اصلاح دستی',
              value: Math.max(0, contribution.manualCostAdjustmentToman),
              color: '#64748b',
            },
          ]}
        />
  ...
  return (
    <div
      className={cn(
        ...
        strong && 'border-t border-[var(--admin-color-border)] pt-3',
      )}
    >
      <dt
        className={cn(
          'text-[var(--admin-color-muted)]',
          strong && 'font-bold text-[var(--admin-color-ink)]',
        )}
      >
        {label}
      </dt>
      <dd
        className={cn(
          ...
          value < 0 && 'text-[var(--admin-color-danger)]',
  ...
  return (
    <article className="rounded-[var(--admin-radius-lg)] border border-[var(--admin-color-border)] bg-white p-3 shadow-[var(--admin-shadow-sm)]">
      <div className="flex items-start justify-between gap-3">
        ...
        <Badge tone={signedTone(supplier.netLiabilityToman)}>
          {formatAdminToman(supplier.netLiabilityToman)}
        </Badge>
      </div>
      <dl className="mt-3 grid grid-cols-2 gap-3 border-y border-[var(--admin-color-border)] py-3 text-xs">
        ...
        <SmallStat
          label="نقد پرداختی دوره"
          value={formatAdminToman(supplier.totalSupplierCashPaidToman)}
        />
...
const supplierColumns: readonly DataTableColumn<FinanceSupplierRow>[] = [
  {
    id: 'supplier',
    header: 'تأمین‌کننده',
    cell: (row) => <span className="font-bold">{row.supplierName}</span>,
  },
  {
    id: 'open',
    header: 'بدهی باز',
    align: 'end',
    cell: (row) => formatAdminToman(row.openPayableToman),
  },
  {
    id: 'credit',
    header: 'اعتبار و کسر',
    align: 'end',
    cell: (row) => formatAdminToman(row.creditOffsetToman),
  },
  {
    id: 'liability',
    header: 'تعهد خالص',
    align: 'end',
    cell: (row) => (
      <Badge tone={signedTone(row.netLiabilityToman)}>
        {formatAdminToman(row.netLiabilityToman)}
      </Badge>
    ),
  },
  {
    id: 'cash',
    header: 'نقد پرداختی دوره',
    align: 'end',
    visibility: 'lg',
    cell: (row) => formatAdminToman(row.totalSupplierCashPaidToman),
  },
  {
    id: 'settlements',
    header: 'تسویه دوره',
    align: 'center',
    visibility: 'lg',
    cell: (row) => formatAdminInteger(row.settlementCount),
  },
  {
    id: 'action',
    header: '',
    align: 'end',
    cell: () => (
      <ButtonLink href="/supplier-payables" variant="ghost" size="sm">
        جزئیات
      </ButtonLink>
    ),
  },
  ...
  return (
    <section aria-labelledby="financial-suppliers-heading" className="space-y-3">
      <div className="flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
        <div>
          <h2 id="financial-suppliers-heading" className="text-lg font-black">
            وضعیت مالی تأمین‌کنندگان
          </h2>
        ...
        <div className="flex gap-2">
          <ButtonLink href="/supplier-payables" variant="outline" size="sm">
            بدهی‌ها
          </ButtonLink>
          <ButtonLink href="/supplier-settlements" variant="outline" size="sm">
            دوره‌های تسویه
          </ButtonLink>
        </div>
      </div>
      <ResponsiveDataView
        ...
        error={
          data.suppliers.failed
            ? { description: 'داده تجمیعی تأمین‌کنندگان دریافت نشد.' }
            : undefined
        }
      />
    </section>
...
function ReconciliationCard({ row }: Readonly<{ row: FinanceCostReconciliationRow }>) {
  return (
    <article className="rounded-[var(--admin-radius-lg)] border border-[var(--admin-color-border)] bg-white p-3 shadow-[var(--admin-shadow-sm)]">
      ...
      <div className="mt-3 border-y border-[var(--admin-color-border)] py-3">
        <MissingCostBadges costs={row.missingCosts} />
      </div>
      <p className="mt-3 text-xs text-[var(--admin-color-muted)]">
        پرداخت: {row.paidAt ? formatAdminDateTime(row.paidAt) : 'ثبت نشده'}
      </p>
      <ButtonLink href="/orders" variant="ghost" size="sm" className="mt-2 w-full">
        رفتن به سفارش‌ها
      </ButtonLink>
    </article>
  );
...
const reconciliationColumns: readonly DataTableColumn<FinanceCostReconciliationRow>[] = [
  {
    ...
    cell: (row) => (
      <div>
        ...
        <p className="mt-1 text-xs text-[var(--admin-color-muted)]">{row.orderStatus}</p>
      </div>
    ),
  },
  {
    ...
    header: 'زمان پرداخت',
    visibility: 'md',
    cell: (row) => (row.paidAt ? formatAdminDateTime(row.paidAt) : '—'),
  },
  {
    id: 'missing',
    header: 'هزینه‌های ثبت‌نشده',
    cell: (row) => <MissingCostBadges costs={row.missingCosts} />,
  },
  {
    id: 'snapshot',
    header: 'Snapshot مالی',
    visibility: 'lg',
    cell: (row) => (
      <Badge tone={row.financeSnapshotReady ? 'success' : 'danger'}>
        {row.financeSnapshotReady ? 'آماده' : 'ناقص'}
      </Badge>
    ),
  },
  {
      ...
      <div className="flex items-end justify-between gap-4">
        <div>
          <div className="flex flex-wrap items-center gap-2">
            <h2 id="cost-reconciliation-heading" className="text-lg font-black">
              کنترل ثبت هزینه سفارش
            </h2>
            {report ? (
              <Badge tone={report.count > 0 ? 'warning' : 'success'}>
                {formatAdminInteger(report.count)} سفارش نیازمند بررسی
              </Badge>
            ) : null}
          </div>
          <p className="mt-1 text-xs leading-6 text-[var(--admin-color-muted)]">
      ...
      <ResponsiveDataView
        ...
        error={
          data.reconciliation.failed
            ? { description: 'فهرست کنترل هزینه سفارش‌ها دریافت نشد.' }
            : undefined
        }
      />
    </section>
      ...
      <header className="flex flex-col gap-4 border-b border-[var(--admin-color-border)] pb-6 sm:flex-row sm:items-end sm:justify-between">
        <div>
          ...
          <p className="mt-2 max-w-3xl text-sm leading-7 text-[var(--admin-color-muted)]">
            جریان نقدی، سود مشارکت، هزینه‌های سفارش و وضعیت تعهد تأمین‌کنندگان را از یک نمای خواندنی
            بررسی کنید.
          </p>
        </div>
      ...
      <div className="space-y-6 pt-6">
        {hasPartialFailure ? (
          <Alert tone="warning" title="گزارش با داده ناقص نمایش داده شده است">
            یک یا چند منبع مالی پاسخ معتبر نداده‌اند. بخش‌های در دسترس مستقل نمایش داده شده‌اند؛
            برای تصمیم مالی ابتدا صفحه را دوباره بارگذاری کنید.
          </Alert>
        ) : null}

        <section
          aria-label="شاخص‌های کلیدی مالی"
        ...
        >
          <KpiCard
            ...
            description={
              management
                ? `${formatAdminInteger(management.paidOrderCount)} سفارش پرداخت‌شده`
                : 'داده در دسترس نیست'
            }
            icon="finance"
            tone="success"
          ...
          <KpiCard
            ...
            value={
              contribution ? formatAdminToman(contribution.contributionAfterRefundsToman) : '—'
            }
            description={
              contribution
                ? `${formatAdminInteger(contribution.costEntryCount)} رکورد هزینه`
                : 'داده در دسترس نیست'
            }
            icon="pulse"
            tone={
              contribution && contribution.contributionAfterRefundsToman < 0 ? 'danger' : 'primary'
            }
          />
          <KpiCard
          ...
          <KpiCard
            ...
            value={
              management ? formatAdminToman(management.supplierPosition.netLiabilityToman) : '—'
            }
