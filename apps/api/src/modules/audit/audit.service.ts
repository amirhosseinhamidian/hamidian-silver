...
export class AuditService {
  ...
  async record(input: RecordAuditLogInput): Promise<void> {
    ...
    await this.prisma.$executeRaw`
      INSERT INTO "audit_logs" (
        "actorUserId", "action", "resource", "resourceId", "method", "path",
        "statusCode", "outcome", "ipAddress", "userAgent", "requestId", "durationMs", "metadata"
      ) VALUES (
        ${input.actorUserId}::uuid, ${input.action}, ${input.resource}, ${input.resourceId}::uuid,
        ${input.method}, ${input.path}, ${input.statusCode}, ${input.outcome}::"AuditOutcome",
        ${input.ipAddress}, ${input.userAgent}, ${input.requestId}, ${input.durationMs}, ${metadata}::jsonb
      )
    `;
  }
  ...
  async list(query: ListAuditLogsQueryDto = {}) {
    ...
    const [items, summaries, resources] = await Promise.all([
      this.prisma.$queryRaw<AuditRow[]>`
        SELECT
          ...
          a."action", a."resource", a."resourceId", a."method", a."path",
    ...
    return {
      items: items.map((item) => ({
        ...
        actor: {
          ...
          name: [item.actorFirstName, item.actorLastName].filter(Boolean).join(' ').trim() || null,
        },
        action: item.action,
